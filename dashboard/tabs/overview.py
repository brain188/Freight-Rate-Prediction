...
def _volume_chart(daily) -> html.Div:
    ...
    figure = go.Figure()
    figure.add_bar(
        x=daily["day"], y=daily["n_predictions"],
        name="predicted", marker_color=TEAL, opacity=0.35,
    )
    figure.add_bar(
        x=daily["day"], y=daily["n_scored"],
        name="outcome received", marker_color=TEAL,
    )
    figure.update_layout(
        **CHART_LAYOUT, barmode="overlay", height=280,
...
def _accuracy_chart(daily) -> html.Div:
    ...
    figure = go.Figure()
    figure.add_scatter(
        x=scored["day"], y=scored["mape"], mode="lines+markers",
        name="live MAPE", line={"color": TEAL, "width": 2.5}, marker={"size": 5},
    )
    figure.add_hline(
        y=1.88, line_dash="dash", line_color=MUTED,
        annotation_text="holdout 1.88%", annotation_position="right",
    )
    figure.update_layout(
...
def _signals(traffic: dict, drift: dict) -> html.Div:
    ...
    if drift.get("drifted_features"):
        notes.append(
            "Features that have moved: " + ", ".join(drift["drifted_features"])
        )

    if not notes:
        return card([
            section("Signals"),
            html.Div("Nothing to act on. Traffic matches the training data.",
